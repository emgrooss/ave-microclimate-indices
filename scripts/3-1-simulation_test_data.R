...
for(i in files){
  ...
  if(e == 0 && v == 0){
    d_a = mean(climates$micro, na.rm=TRUE) - mean(climates$macro, na.rm=TRUE)
     ...
     d_v = climates %>% dplyr::group_by(month = lubridate::yday(date)) %>%
      dplyr::summarize( v_obs_monthly = (sd(micro, na.rm = TRUE) - sd(macro, na.rm = TRUE)) / sd(macro, na.rm = TRUE) ) %>%
      summarize(v_obs = mean(v_obs_monthly)) %>% as.numeric()
     
     p95 = unname(quantile(climates$macro, 0.95, na.rm=TRUE))
     d_emax = climates %>% filter(macro > p95) %>% 
       mutate(d_emax = (micro-macro)/(p95-macro) )  %>% summarise(mean(d_emax)) %>% as.numeric()
     
     p5 = unname(quantile(climates$macro, 0.05, na.rm=TRUE))
     d_emin = climates %>% filter(macro < p5) %>% 
       mutate(d_emin = (micro-macro)/(p5-macro)) %>% summarise(mean(d_emin)) %>% as.numeric()    
