# ===============================================================================================================================
# Title       : 0-index-functions.R
# Description : Functions to calculate microclimate indices
# Author      : ###
# Affiliation : ###
# Contact     : ###
# Date        : 2025-09-24
...
sd_offset_mean_daily <- function(time.index, macroclimate, microclimate) {
  ...
  df <- df %>%
    dplyr::group_by(day = lubridate::day(time.index)) %>%
    dplyr::summarize(
      sd_macro = sd(macroclimate, na.rm = TRUE),
      sd_micro = sd(microclimate, na.rm = TRUE)
    )
...
amplitude_offset_mean_daily <- function(time.index, macroclimate, microclimate) {
  ...
  df <- df %>%
    ...
    dplyr::summarize(
      max_macro = max(macroclimate, na.rm = TRUE), min_macro = min(macroclimate, na.rm = TRUE),
...
offset_of_maxima_mean_daily <- function(time.index, macroclimate, microclimate, percentile = 1.00) {
  ...
  df <- df %>%
    dplyr::group_by(day = lubridate::yday(time.index)) %>%
    dplyr::summarize(
      max_macro = unname(quantile(macroclimate, percentile, na.rm = TRUE)),
...
p95_daily_maxima_offset <- function(time.index, macroclimate, microclimate) {
  ...
  df <- df %>%
    dplyr::group_by(day = lubridate::yday(time.index)) %>%
    dplyr::summarize(
      max_macro = max(macroclimate, na.rm = TRUE),
...
offset_of_minima_mean_daily <- function(time.index, macroclimate, microclimate, percentile = .05) {
  ...
  df <- df %>%
    dplyr::group_by(day = lubridate::yday(time.index)) %>%
    dplyr::summarize(
      min_macro = unname(quantile(macroclimate, percentile, na.rm = TRUE)),
...
p5_daily_minima_offset <- function(time.index, macroclimate, microclimate) {
  ...
  df <- df %>%
    dplyr::group_by(day = lubridate::yday(time.index)) %>%
    dplyr::summarize(
      min_macro = min(macroclimate, na.rm = TRUE),
