...
coordinates_sf = st_as_sf(dd, coords = c("LON", "LAT"), 
        ...
        expr = {
          ...
          if(nrow(d.temp)>0){
            if(length(unique(d.temp$hour))==24){
              ...
              print(head(d.temp))
              write_csv(x = data.frame(code = paste0(ddd$USAF[i],'-',ddd$WBAN[i]),
                                       country = ddd$CTRY[i]),
                        file = paste0('data/1-1-noaa/potential-stations/',
                                      paste0(ddd$USAF[i],'-',ddd$WBAN[i]), 
                                      '.csv'))
            }}
        }, timeout = 30)
    }, TimeoutException = function(ex){cat("Timeout\n")}, error = function(e){cat("Error\n")})
}
...
sample_by_country = sites %>% group_by(CTRY) %>% slice_sample(n=1)
...
sites_sample = data.frame(code = character(),
                          country = character())
